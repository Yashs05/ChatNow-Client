...
const SignUp = ({ navigation }) => {
    ...
    const handleSignUp = () => {
        ...
        if (!creds.name.trim() || !creds.email.trim() || !creds.username.trim() || creds.password.length < 6) {
            setCredsInvalid(true)
        }

        const formData = new FormData()

        formData.append('name', creds.name.trim())
        formData.append('email', creds.email.trim())
        formData.append('username', creds.username.trim())
        formData.append('password', creds.password.trim())
        
        setLoading(true)
        dispatch(registerUser(formData, navigation, setLoading, setError))
    }
            ...
            <ScrollView contentContainerStyle={{ flexGrow: 1 }} style={{ width: '100%' }}>
                <View style={{ width: '100%' }}>
                    ...
                    <View>
                        <TextInput
                            value={creds.name}
                            onChangeText={(value) => setCreds({ ...creds, name: value })}
                            label='Full name'
                            mode='outlined'
                        ...
                        <TextInput
                            ...
                            onChangeText={(value) => setCreds({ ...creds, username: value })}
                            label='Username'
                            mode='outlined'
                            activeOutlineColor='#167bd1'
                            textColor='black'
